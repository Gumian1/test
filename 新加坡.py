# -*- coding: utf-8 -*-
"""
Created on Mon Sep 26 23:50:07 2022
...
def main():
    ...
    body.get("https://www.zaobao.com/search-result?k=%E4%B8%AD%E5%9B%BD")
    # 滑动页面到底部
    for i in range(10):
    ...
    urls = html_str.xpath(r'//div[@class ="row reset-row-margin d-lg-block d-none"]/a/@href')
    print("urls在这：")
    print(urls)

    for url in urls:
        url_1 = 'https://www.zaobao.com'+ url
        html = request(url_1)
        if html == 0:
            continue
        else:
            num += 1
            print("正在爬取第" + str(num) + "个：" + url_1)
            content(html)
...
def content(html):
    ...
    # 获取新闻内容
    try:
        texts = html.xpath("/html/body/div[5]/div[2]/div[2]/div[2]/div/article/p")      
        text = ""
        for i in texts:
    ...
    if not html.xpath("/html/body/div[5]/div[2]/div[2]/div[1]/div/div[1]/div[1]/div[2]/span/text()"):
        time=''
    else:
        time=html.xpath("/html/body/div[5]/div[2]/div[2]/div[1]/div/div[1]/div[1]/div[2]/span/text()")
    print('time is：')
    print(time)
